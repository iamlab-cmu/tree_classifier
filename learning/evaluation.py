...
def plot_confusion_matrix(y_true, y_pred, classes, cfg, suffix=""):
    ...
    # Remove plt.show() to prevent displaying
    plt.close()  # Close the figure to free memory


def save_misclassified_samples(val_loader, model, val_preds, val_labels, cfg, device='cuda'):
    """
    Save misclassified samples to a folder for analysis.
    """
    # Create a simple, reliable path structure in the current working directory
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # Create base "missed" directory in current working directory
    ...
    # Create a CSV to store information about misclassified samples
    csv_path = os.path.join(misclassified_dir, 'misclassified_samples.csv')
    with open(csv_path, 'w') as f:
        f.write("sample_idx,image_file,audio_file,true_label,predicted_label,saved_image_path,saved_spectrogram_path\n")
    ...
    # Create folders for each type of misclassification
    created_folders = []
    ...
    print(f"Created {len(created_folders)} folders for different misclassification types")
    
    # Process each misclassified sample
    successful_saves = 0
    for idx in misclassified_indices:
        if idx >= len(all_samples):
            print(f"Warning: Index {idx} out of range for all_samples (length: {len(all_samples)})")
            continue
        ...
        # Create folder path for this type of misclassification
        folder_path = os.path.join(misclassified_dir, f"{true_class}_as_{pred_class}")
        print(f"Saving to folder: {folder_path}")
        
        # Save the image with labels overlaid
        ...
        # Add true label (green)
        true_text = f"True: {true_class}"
        text_size = cv2.getTextSize(true_text, font, font_scale, thickness)[0]
        text_x = 10
        text_y = 30
        cv2.putText(img_bgr, true_text, (text_x, text_y), font, font_scale, (0, 255, 0), thickness)
        
        # Add predicted label (red)
        pred_text = f"Pred: {pred_class}"
        text_y += text_size[1] + 10
        cv2.putText(img_bgr, pred_text, (text_x, text_y), font, font_scale, (0, 0, 255), thickness)
        ...
        # Save the audio spectrogram with better visualization
        ...
        if len(non_zero_cols) > 0:
            # Find start and end of content with some padding
            ...
            # Plot the full spectrogram in a smaller subplot
            plt.subplot(2, 1, 1)
            plt.imshow(audio_np, aspect='auto', origin='lower')
            plt.title("Full Spectrogram")
            plt.colorbar(format='%+2.0f dB')
            
            # Plot the trimmed spectrogram in a larger subplot
            plt.subplot(2, 1, 2)
            ...
            # If no significant content found, just show the full spectrogram
        ...
        # Append to CSV - Fixed to handle dataframe access properly
        with open(csv_path, 'a') as f:
            # Safely get original file paths if available
            try:
                # Convert idx to int to avoid int64 type issues
                idx_int = int(idx)
                
                # Check if dataset has dataframe attribute
            ...
            f.write(f"{idx},{image_file},{audio_file},{true_class},{pred_class},{img_path},{spec_path}\n")
    ...
    # Add debug output to list directory contents
    ...
    # Try to open one of the images to verify it's valid
    ...
    if cfg.wandb.enabled:
        try:
            import pandas as pd
            wandb.log({"misclassified_samples_csv": wandb.Table(dataframe=pd.read_csv(csv_path))})
        except Exception as e:
