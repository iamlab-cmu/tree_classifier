...
def initialize_wandb(cfg, run_name_suffix=None):
    ...
    # Generate a unique run name with timestamp and optional suffix
    if run_name_suffix:
...
def setup_robot_humming_dir(cfg):
    ...
    if cfg.data.get('augmentations', {}).get('audio_robot_humming', False):
        ...
        robot_humming_path = to_absolute_path(cfg.data.augmentations.get('robot_humming_audio_path', ''))
        if not os.path.exists(robot_humming_path):
            ...
            # Create a README file to explain how to use this directory
            readme_path = os.path.join(robot_humming_path, "README.txt")
            with open(readme_path, "w") as f:
                f.write("Robot Humming Samples Directory\n")
                f.write("==============================\n\n")
                f.write("Place robot humming audio samples (WAV, MP3, OGG) in this directory.\n")
                f.write("These samples will be used for data augmentation to mix with training audio.\n")
                f.write("Samples should ideally be robot motor/movement sounds without contact sounds.\n")
                
            print(f"Created directory: {robot_humming_path}")
            print(f"Added README file: {readme_path}")
            print("Please add robot humming audio samples to this directory for augmentation to work.")
...
def log_classification_metrics(report, category_classes, binary_mode=False, prefix=""):
    ...
    # Create a more comprehensive metrics table with precision and recall
...
def convert_to_binary_classification(labels, preds):
    ...
    # 'leaf', 'twig', 'trunk' (0, 1, 2) -> 'contact' (0)
    # 'ambient' (3) -> 'no-contact' (1)
...
def update_num_classes(cfg):
    ...
    if cfg.data.get('use_binary_classification', False):
        ...
        if len(cfg.data.classes) != cfg.model.num_classes:
            print(f"Warning: Mismatch between number of classes in data config ({len(cfg.data.classes)}) "
