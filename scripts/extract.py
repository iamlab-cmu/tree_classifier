...
# Now import segment_audio function
...
class RosBagExtractor:
    def __init__(self, bag_path, output_dir=None):
        ...
        # Add audio segmentation storage
    ...
    def extract_audio(self):
        ...
        # Create temp file to get sample rate
        ...
        # Print audio duration using detected sample rate
        duration = len(audio_array) / sample_rate
        print(f"Audio duration: {duration:.2f} seconds")
        
        # Preprocess audio with detected sample rate
        ...
        if audio_data and timestamp:
            ...
            # Create waveform visualization in a separate file
            plt.figure(figsize=(15, 5))
            time = np.arange(len(audio_array)) / sample_rate
            # Average the channels for a single waveform display
            ...
            # Create spectrogram visualization in a separate file
            plt.figure(figsize=(15, 5))
            # Create a single combined spectrogram instead of separate channels
    ...
    def process_audio_segments(self):
        ...
        try:
            # Find the audio file in the audio directory
            ...
            # Load audio data for visualization - preserve stereo channels
            self.audio_data, self.audio_sr = librosa.load(audio_path, sr=None, mono=False)  # Use original sample rate, keep stereo
            
            # If stereo, use the average of both channels for visualization
            if len(self.audio_data.shape) > 1 and self.audio_data.shape[0] == 2:
                # Keep stereo data for other processing, but use average for visualization
                self.audio_data_mono = np.mean(self.audio_data, axis=0)
            else:
                # If already mono, just use as is
                self.audio_data_mono = self.audio_data
            
            # Load config
            cfg = OmegaConf.load('scripts/config/config.yaml')
            
            # Get segments using the segment_audio function with config parameters
            ...
            # Properly handle the return tuple
            if isinstance(segments_and_force, tuple):
                ...
                contact_segments = sum(1 for _, _, is_contact, _, _ in self.audio_segments if is_contact)
                ...
                contact_duration = sum((end-start)/self.audio_sr for start, end, is_contact, _, _ in self.audio_segments if is_contact)
                non_contact_duration = sum((end-start)/self.audio_sr for start, end, is_contact, _, _ in self.audio_segments if not is_contact)
                
                print(f"Contact duration: {contact_duration:.2f}s")
                print(f"Non-contact duration: {non_contact_duration:.2f}s")
                
                for i, (start, end, is_contact, windows, _) in enumerate(self.audio_segments):
                    # Use the same sample rate for time conversion
                    segment_type = "Contact" if is_contact else "Non-contact"
                    print(f"Segment {i+1}: {segment_type}, start={start/self.audio_sr:.2f}s, end={end/self.audio_sr:.2f}s, duration={(end-start)/self.audio_sr:.2f}s")
            else:
                ...
                # Count contact and non-contact segments
                contact_segments = sum(1 for _, _, is_contact, _ in self.audio_segments if is_contact)
                non_contact_segments = len(self.audio_segments) - contact_segments
                ...
                # Calculate total duration for each type
                contact_duration = sum((end-start)/self.audio_sr for start, end, is_contact, _ in self.audio_segments if is_contact)
                non_contact_duration = sum((end-start)/self.audio_sr for start, end, is_contact, _ in self.audio_segments if not is_contact)
                
                print(f"Contact duration: {contact_duration:.2f}s")
                print(f"Non-contact duration: {non_contact_duration:.2f}s")
                
                for i, (start, end, is_contact, windows) in enumerate(self.audio_segments):
                    # Use the same sample rate for time conversion
                    segment_type = "Contact" if is_contact else "Non-contact"
                    print(f"Segment {i+1}: {segment_type}, start={start/self.audio_sr:.2f}s, end={end/self.audio_sr:.2f}s, duration={(end-start)/self.audio_sr:.2f}s")
    ...
    def create_video(self, fps=None):
        ...
        # Use calculated FPS if none provided
        if fps is None:
            fps = self.fps
...
def preprocess_audio(audio_data, sample_rate):
    ...
    # Load config for noise file path
    cfg = OmegaConf.load('scripts/config/config.yaml')
    ...
    if not noise_file or not os.path.exists(noise_file):
        raise Exception(f"Noise file not found. Please ensure the noise file exists at the path specified in config.yaml")
    
    try:
        # Concatenate all audio chunks, maintaining stereo channels
        audio_array = np.concatenate(audio_data, axis=0)
        
        # Convert stereo to mono by averaging channels for noise reduction
        ...
        # Convert back to stereo by duplicating the cleaned mono channel
        cleaned_stereo = np.stack([reduced_noise, reduced_noise], axis=1)
        
        # Return as list with single array to maintain compatibility
