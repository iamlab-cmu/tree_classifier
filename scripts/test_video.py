...
def extract_images_from_bag(bag_path, output_dir):
    ...
    # Clean any existing images from previous runs to avoid duplication
    existing_images = glob.glob(os.path.join(image_dir, '*.jpg'))
    if existing_images:
        print(f"Removing {len(existing_images)} existing images...", end=" ", flush=True)
    ...
    # Find the first image topic in the bag
    image_topic = None
    ...
    # Extract images and calculate FPS
    with rosbag.Bag(bag_path, 'r') as bag:
        # First pass: collect frame timestamps for FPS calculation
        frame_times = []
        ...
        # Second pass: extract image frames - now using a set to track unique timestamps
...
def extract_audio_from_bag(bag_path, output_dir):
    ...
    # Find the first audio topic in the bag
    audio_topic = None
    with rosbag.Bag(bag_path, 'r') as bag:
        for topic, info in bag.get_type_and_topic_info().topics.items():
    ...
    # Save as WAV file - using standard 44100Hz sample rate
    sample_rate = 44100
...
def create_video_from_bag(bag_path, output_dir=None, fps=None):
    ...
    # Use the calculated FPS or user-specified FPS
    if fps is not None:
        video_fps = fps
    ...
    # Get the image files for creating the video
    ...
    # Get the image files and ensure they're properly sorted
    ...
    try:
        ...
        if audio_file and os.path.exists(audio_file):
            ...
            print(f"Warning: Audio file {audio_file} not found, creating video without audio")
        
        # Write the result with minimal logging
        with suppress_stdout():
            # If the output file already exists, remove it to avoid issues
...
def main():
    parser = argparse.ArgumentParser(description='Create video from ROS bag file(s) without preprocessing')
    parser.add_argument('path', help='Path to ROS bag file or directory containing bag files')
    parser.add_argument('--output', '-o', help='Output directory (default: output/bag_name)')
    parser.add_argument('--fps', type=float, help='Override FPS for the output video')
    parser.add_argument('--recursive', '-r', action='store_true', help='Search recursively for bag files in subdirectories')
    ...
    # Process a directory of bag files
        ...
        for i, bag_path in enumerate(bag_files):
            ...
            try:
                ...
                if result:
                    print(f"Successfully created video from {os.path.basename(bag_path)}")
                    success_count += 1
                else:
