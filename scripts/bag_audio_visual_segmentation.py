...
def extract_audio_from_bag(bag_path, save_to_file=None, cfg=None):
    ...
    try:
        with rosbag.Bag(bag_path, 'r') as bag:
            ...
            # If not found, look for topics with AudioData in their type
            if not audio_topic:
            ...
            print(f"Selected audio topic: {audio_topic} with message type: {topics_info[1][audio_topic].msg_type}")
            ...
            for _, msg, _ in bag.read_messages(topics=[audio_topic]):
                ...
                if audio_msg_count == 1:
                    # Get a sample message to check its type
                    print(f"Audio message type: {type(msg).__name__}")
                    # Only print the most relevant attributes
                    relevant_attrs = [attr for attr in dir(msg) if not attr.startswith('_') and attr not in 
            ...
            if audio_data:
                try:
                    ...
                    # Check if we should normalize audio based on config
                    do_norm = True  # Default to True for backward compatibility
                    if cfg and hasattr(cfg, 'preprocessing') and hasattr(cfg.preprocessing, 'do_norm'):
                        do_norm = cfg.preprocessing.do_norm
                        print(f"Audio normalization is {'enabled' if do_norm else 'disabled'} as per configuration")
                    else:
                        print("No do_norm setting found in config, defaulting to enabled")
                    ...
                    if do_norm:
                        ...
                        if rms_level > 0:  # Avoid division by zero
                            ...
                            print(f"Applied RMS normalization: original RMS={rms_level:.6f}, target RMS={target_rms:.6f}")
                        else:
                            print("Warning: Audio has zero RMS level, skipping normalization")
                    else:
                        print("Skipping audio normalization as it's disabled in the configuration")
                    ...
                    # Save audio data to file using scipy.io.wavfile which is more robust
                    try:
                        from scipy.io import wavfile
                        print(f"Saving audio data to {temp_file} with scipy (sample rate: {sample_rate})")
                        wavfile.write(temp_file, sample_rate, all_audio)
                        
                        # Verify the file was created successfully
                        if os.path.isfile(temp_file) and os.path.getsize(temp_file) > 0:
                            # Load the file to get some statistics
                            try:
                                y, sr = librosa.load(temp_file, sr=None)
                                print(f"Successfully extracted audio: {len(y)} samples at {sr}Hz")
                                print(f"Audio statistics: min={np.min(y):.6f}, max={np.max(y):.6f}, mean={np.mean(y):.6f}")
                                
                                # Check if the amplitude is too low
                                if np.max(np.abs(y)) < 0.01:
                                    print("WARNING: Extracted audio has very low amplitude - contact detection may fail")
                            except Exception as e:
                    ...
                    print(f"Audio data shapes: {[d.shape if isinstance(d, np.ndarray) else type(d) for d in audio_data]}")
                    return None
            else:
...
def process_bag(bag_path, output_dir, window_length_seconds=1.0, window_stride_seconds=0.1, cfg=None, is_robot=False):
    ...
    try:
        ...
        # Initialize metadata list to store information about processed windows
        metadata = []
        ...
        # Get bag name for file naming
        bag_name = os.path.splitext(os.path.basename(bag_path))[0]
        
        # Try to determine the category from the bag name
        standard_category = map_to_category(bag_name, cfg)
        
        # Skip this bag if no category could be determined
        if standard_category is None:
            print(f"Skipping {bag_name} as no category could be determined")
            print(f"Make sure the bag name contains one of the categories defined in config.yaml")
            return []
            
        print(f"Mapped {bag_name} to category: {standard_category}")
        
        # Extract audio from the bag - now passing the config
        ...
        print(f"Audio stats: min={np.min(data):.6f}, max={np.max(data):.6f}, mean={np.mean(data):.6f}")
        
        # Initialize noise data and sample rate variables
        noise_data = None
        noise_sr = None
        
        # Check if denoising is enabled in the config
        enable_denoising = True  # Default to True for backward compatibility
        if cfg and hasattr(cfg, 'preprocessing') and hasattr(cfg.preprocessing, 'enable_denoising'):
            enable_denoising = cfg.preprocessing.enable_denoising
            print(f"Denoising is {'enabled' if enable_denoising else 'disabled'} as per configuration")
        else:
            print("No enable_denoising setting found in config, defaulting to enabled")
        
        # Skip noise file loading if denoising is disabled
        if not enable_denoising:
            print("Skipping denoising as it's disabled in the configuration")
            segmentation_audio_data = data
        else:
            # Get appropriate noise file path from config based on whether it's robot or probe data
            noise_file = None
            if cfg and hasattr(cfg, 'preprocessing'):
                if is_robot and hasattr(cfg.preprocessing, 'robot_noise_file'):
                    ...
                    # Fallback to generic noise file
                ...
                if noise_file and not os.path.isabs(noise_file):
                    # If using Hydra, get the original working directory
                    if 'hydra' in sys.modules:
                        ...
                        if os.path.exists(absolute_noise_file):
                            ...
                            print(f"Warning: Noise file not found at {absolute_noise_file}")
                            
                            # Try to find the file in the current working directory
                            if os.path.exists(noise_file):
                                print(f"Found noise file in current directory: {noise_file}")
                            else:
                                print(f"Warning: Noise file not found in current directory either: {noise_file}")
                                
                                # Fall back to the other type of noise file if available
                                if is_robot and hasattr(cfg.preprocessing, 'probe_noise_file'):
                                    fallback_file = cfg.preprocessing.probe_noise_file
                                    if not os.path.isabs(fallback_file):
                                        fallback_file = os.path.join(original_cwd, fallback_file)
                                    if os.path.exists(fallback_file):
                                        noise_file = fallback_file
                                        print(f"Falling back to probe noise file: {noise_file}")
                                elif not is_robot and hasattr(cfg.preprocessing, 'robot_noise_file'):
                                    fallback_file = cfg.preprocessing.robot_noise_file
                                    if not os.path.isabs(fallback_file):
                                        fallback_file = os.path.join(original_cwd, fallback_file)
                                    if os.path.exists(fallback_file):
                                        noise_file = fallback_file
                                        print(f"Falling back to robot noise file: {noise_file}")
            else:
                # Fallback to default
                default_file = './robot_humming.wav' if is_robot else './probe_humming.wav'
                noise_file = default_file
                print(f"Using default noise file: {noise_file}")
            
            # Apply denoising if noise file exists and is accessible
            use_denoising = False
            if noise_file and os.path.exists(noise_file):
                try:
                    ...
                    # Initialize these to prevent variable reference errors
                    noise_data = None
                    noise_sr = None
            else:
            ...
            if use_denoising and noise_data is not None and noise_sr is not None:
                try:
                    ...
                    # Use denoised audio for segmentation without saving full file
                    segmentation_audio_data = denoised_data
                except Exception as e:
                    print(f"Error during denoising: {e}")
                    # Fall back to original audio if denoising fails
                    segmentation_audio_data = data
            else:
                print("No valid noise file available for denoising, using original audio")
                segmentation_audio_data = data
        
        # Create a temporary in-memory file for segmentation
        with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as temp_segmentation_file:
            temp_segmentation_path = temp_segmentation_file.name
            sf.write(temp_segmentation_path, segmentation_audio_data, sr)
        
        # Create a bridge for image conversion
        bridge = CvBridge()
        ...
        with rosbag.Bag(bag_path, 'r') as bag:
            ...
            # Find image topics - look for Image message types
            image_topics = []
            for topic, info in topics_info[1].items():
        ...
        # Try to segment using the denoised data
        try:
            # Define the get_config function here to ensure it's in scope
            ...
            # Before calling segment_audio, explicitly print all parameters
            ...
            print(f"non_contact_threshold_factor: {get_config_for_segment('non_contact_threshold_factor')}")
            print(f"enable_squeezing: {get_config_for_segment('enable_squeezing')}")
            print(f"squeeze_factor_seconds: 0.3")
            print(f"dynamic_threshold_offset: {get_config_for_segment('dynamic_threshold_offset')}")
            ...
            # Add detailed output about the returned segments
            ...
            for i, (start, end, is_contact, windows) in enumerate(segments):
                ...
                print(f"Segment {i+1} ({segment_type}): {len(windows)} windows, {duration:.2f} seconds")
                if is_contact:
                    contact_window_total += len(windows)
            print(f"Total contact windows found: {contact_window_total}")
            print("===========================\n")
            
            # Important: Do NOT modify the segments and windows returned by segment_audio
            # Just use them directly as provided by segment_audio
                
        except Exception as e:
            print(f"Error segmenting audio from {bag_path}: {str(e)}")
            print(f"Audio stats: min={np.min(segmentation_audio_data)}, max={np.max(segmentation_audio_data)}, mean={np.mean(segmentation_audio_data)}, shape={segmentation_audio_data.shape}, sr={sr}")
            os.unlink(temp_audio_path)
        ...
        print(f"Found {len(segments)} segments in {bag_path} ({contact_segments} contact, {non_contact_segments} non-contact)")
        
        # Process each segment
        for i, segment in enumerate(segments):
            # Unpack the segment tuple correctly
            start, end, is_contact, windows = segment
            
            # Skip non-contact segments if desired
            # if not is_contact:
            #     continue
            ...
            # CRITICAL: Only use the windows provided by segment_audio
            # Remove any fallback logic to prevent recalculation
            
            # Verify we have windows for this segment
            if not windows or len(windows) == 0:
                print(f"Warning: Segment {i+1} ({segment_type}) has no windows. Skipping.")
                continue
            
            print(f"Processing segment {i+1} ({segment_type}): {len(windows)} windows")
            
            # Process each window
            for j, window_data in enumerate(windows):
                # Save window to WAV file
                window_filename = f"{bag_name}_segment_{i}_window_{j}_{segment_type}.wav"
                window_path = os.path.join(audio_dir, window_filename)
                sf.write(window_path, window_data, sr)
                
                # Find the middle timestamp for the audio window
                window_middle_time = (start + j * (window_stride_seconds if is_contact else window_stride_seconds * 2) * sr + len(window_data) / 2) / sr
                
                # Find the image closest to the middle timestamp
                try:
                    closest_image = None
                    min_time_diff = float('inf')
                    
                    # Search in all image topics
                    ...
                    # Save the image
                    img_filename = f"{bag_name}_segment_{i}_window_{j}_{segment_type}.jpg"
                    img_path = os.path.join(image_dir, img_filename)
                    cv2.imwrite(img_path, cv_image)
                    
                    # Add metadata entry with only the required columns and standardized category
                    metadata.append({
                        'audio_file': os.path.join("audio", window_filename),
                        'image_file': os.path.join("images", img_filename),
                        'category': "ambient" if not is_contact else standard_category
                    })
        ...
        # Save debug audio files if debug mode is enabled
        if hasattr(cfg, 'debug') and hasattr(cfg.debug, 'enabled') and cfg.debug.enabled:
            debug_dir = os.path.join(output_dir, "debug")
            os.makedirs(debug_dir, exist_ok=True)
            
            # Save the original extracted audio
            ...
            # Save the denoised audio used for segmentation
            ...
            # Run segment_audio directly on both to compare
            ...
            contact_segments_orig = sum(1 for _, _, is_contact, _ in orig_segments if is_contact)
            print(f"Contact segments: {contact_segments_orig}, Non-contact segments: {len(orig_segments) - contact_segments_orig}")
        
        # At the end of the processing loop, before returning metadata:
        ...
        # Verify the contact window count matches what segment_audio found
        if contact_files != contact_window_total:
            print(f"WARNING: Mismatch between detected contact windows ({contact_window_total}) and processed files ({contact_files})")
            print("This suggests the window handling code is not working as expected")
        
        return metadata
    except Exception as e:
        print(f"Error processing bag {bag_path}: {str(e)}")
        # Make sure to clean up any temporary files that might have been created
        if 'temp_audio_path' in locals() and os.path.exists(temp_audio_path):
            os.unlink(temp_audio_path)
        if 'temp_segmentation_path' in locals() and os.path.exists(temp_segmentation_path):
            os.unlink(temp_segmentation_path)
        return []
...
def process_bags(cfg: DictConfig):
    ...
    # Get output directories for probe and robot datasets
    ...
    # Check if denoising is specified as a command-line argument
    if len(sys.argv) > 1:
        for arg in sys.argv:
            if arg.startswith("preprocessing.enable_denoising="):
                denoising_value = arg.split("=")[1].lower()
                if denoising_value in ["true", "false"]:
                    # Update the config with the command-line specified value
                    if not hasattr(cfg, 'preprocessing'):
                        cfg.preprocessing = {}
                    cfg.preprocessing.enable_denoising = (denoising_value == "true")
                    print(f"Denoising {'enabled' if cfg.preprocessing.enable_denoising else 'disabled'} via command-line argument")
    ...
    if not os.path.isdir(probe_bag_dir):
        ...
        probe_bags = glob.glob(os.path.join(probe_bag_dir, '**/*.bag'), recursive=True) + \
                    glob.glob(os.path.join(probe_bag_dir, '**/*.BAG'), recursive=True)
        
        if not probe_bags:
            print(f"WARNING: No probe bag files (*.bag, *.BAG) found in {probe_bag_dir}")
        else:
            print(f"Found {len(probe_bags)} probe bag files")
            # Process all probe bags
            probe_metadata = []
            ...
            if probe_metadata:
                ...
                print("WARNING: No metadata collected from probe bags, dataset not created")
    ...
    if not os.path.isdir(robot_bag_dir):
        ...
        robot_bags = glob.glob(os.path.join(robot_bag_dir, '**/*.bag'), recursive=True) + \
                    glob.glob(os.path.join(robot_bag_dir, '**/*.BAG'), recursive=True)
        
        if not robot_bags:
            print(f"WARNING: No robot bag files (*.bag, *.BAG) found in {robot_bag_dir}")
        else:
            print(f"Found {len(robot_bags)} robot bag files")
            # Process all robot bags
            robot_metadata = []
            for bag_path in tqdm(robot_bags, desc="Processing robot bags", disable=debug_mode):
                # For debugging, you can process just one specific bag
            ...
            if robot_metadata:
                ...
                print("WARNING: No metadata collected from robot bags, dataset not created")
                print("Check for issues in the process_bag function or bag contents")

def map_to_category(bag_name, cfg=None):
    """Map bag name to a standard category."""
    # Get available categories from config if provided
    ...
    # Convert bag name to lowercase for case-insensitive matching
    ...
    # If no match found, provide detailed warning
    print(f"WARNING: Could not determine category for bag: {bag_name}")
    print(f"  Available categories: {categories}")
    print("  Bags must have a category name in their filename (e.g., 'leaf_sample.bag')")
    print("  If this is intentional, you can ignore this warning.")
    
    # Special handling for 'ambient' category
    ambient_keywords = ['ambient', 'background', 'noncontact']
    if 'ambient' in categories and any(keyword in bag_name_lower for keyword in ambient_keywords):
        print(f"  Assigning to 'ambient' category based on keywords: {ambient_keywords}")
        return 'ambient'
...
def check_segments(segments, audio_data, sr):
    ...
    for segment in segments:
        ...
        if start < 0 or end > len(audio_data) or start >= end:
            print(f"Error: segment indices out of bounds: {start}, {end}, audio length: {len(audio_data)}")
            return False
...
def visualize_segmentation(audio_data, sr, envelope, thresholds, segments, output_path):
    # ... simplified version of the visualization code from segment_audio.py ...
